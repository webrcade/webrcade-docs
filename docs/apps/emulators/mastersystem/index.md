...
Some Master System cartridges include battery-backed SRAM as a means of preserving state between sessions. The Master System application supports persisting this SRAM state into the browser's local storage or optionally to [cloud-based storage](../../../storage/index.md). The SRAM contents will be persisted to storage whenever the pause screen is displayed (or the game is exited). Therefore, the menu should be displayed periodically for games that support battery-backed SRAM to ensure the state is properly persisted.
...
Two Sega Master System application types are available, each offering different trade-offs in compatibility, features, and system resource requirements. *Libretro Genesis Plus GX* is the default (⭐) and is mapped to the `sms` alias. The default can be overridden globally in [Settings](../../../userguide/settings.md) > *Applications*, or on a per-item basis in the [Feed Editor](../../../editor/index.md).

| __Name__ | __Type__ | __Cheats__ | __Shaders__ | __Retro<br>Achievements__ | __Low<br>CPU__ |
| --- | --- | --- | --- | --- | --- |
| Libretro Genesis Plus GX ⭐ | `retro-genplusgx-sms` | ✅ | ✅ | ✅ | x |
| Genesis Plus GX | `genplusgx-sms` | x | x | x | ✅ |
...
- [Sega Master System (Libretro Genesis Plus GX) GitHub Repository](https://github.com/webrcade/webrcade-app-retro-genplusgx)
- [Sega Master System (Genesis Plus GX) GitHub Repository](https://github.com/webrcade/webrcade-app-genplusgx)
